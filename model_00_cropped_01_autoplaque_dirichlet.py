...
def main():
    ...
    omega.poisson = omega_topo.projection(
        omega.poisson,
        onto=omega_topo.basis("spline", degree=2),
        geometry=omega.x,
        ptype=ptype,
    ...
    export.vtk(
        "model_00_cropped_test_trimmed_lumen_mesh", bezier.tri, points, traction=vals
    )
    ...
    trim = trim_data.reshape(
        [trim_data.shape[0] * trim_data.shape[1] * trim_data.shape[2], 1, 1], order="C"
    ).flatten()
    omega.trim = omega.linbasis.dot(trim)

    # Read Inner Wall Implicit Representation
    trim_lumen_data = np.loadtxt("data\\trim_lumen.txt", skiprows=3).reshape(
        shape, order="F"
    )

    # Build Trimming Function
    trim_lumen = trim_lumen_data.reshape(
        [trim_data.shape[0] * trim_data.shape[1] * trim_data.shape[2], 1, 1], order="C"
    ).flatten()
    ...
    normalizedvonmises, normalizedmeanstress = bezier.eval(
        ["normalizedvonmises", "normalizedmeanstress"] @ omega, lhs=lhs
    )
