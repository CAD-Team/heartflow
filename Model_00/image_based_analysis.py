...
class IM_TriMesh:
	...
	def TrimByBoundingBox(self, bb):
		self.verts, self.tris, self.data = TrimMeshDataByBoundingBox(self.verts, self.tris, self.data, bb)
	...
	def GetAABB(self):
		bounds = [np.min(self.verts,axis=0), np.max(self.verts,axis=0)]
		return IM_AABB(bounds[0], bounds[1])


def TrimMeshDataByBoundingBox(verts, cells, data, bb):
	new_cells = []
	new_verts = []
	new_data = {}
	...
	return np.array(new_verts), np.array(new_cells), new_data
...
def GetAffineTransformation(structured_topo, geom): # this doesnt account for rotations!!!
...
class Heaviside(function.Array):
    @types.apply_annotations
    def __init__(self, *args:function.asarrays):
        shapes = set(arg.shape for arg in args)
        assert len(shapes) == 1, 'pointwise arguments have inconsistent shapes'
        shape, = shapes
        self.args = args
...
def TrimMeshByBoundingBox(topo, geom, aabb):
...
def ProjectSampleTriMesh(fromgeom, totopo, togeom, fromsample):
    ...
    for i in range(len(pts)):
        ...
        ielems[i], proj, min_dist = ProjectPointOntoTriMesh(verts, tris, pts[i])
        print(ielems[i])
        print(proj)
        xis[i] = proj[0][1:3]
...
def DistancePointTriMesh(verts, tris, pt):
	return ProjectPointOntoTriMesh(verts, tris, pt)[2]
