...
def main():
	...
	omega.lmbda = omega_topo.projection(omega.lmbda, onto=omega_topo.basis('spline',degree=2), geometry=omega.x, ptype=ptype, ischeme='gauss{}'.format(2),solver='cg', atol=1e-10, precon='diag')
	...
	#lhs = solver.solve_linear('lhs', residual=K-F, constrain=cons, linsolver='cg', linatol=1e-10, linprecon='diag')
	...
	# sample
	bezier = omega_topo.sample('bezier', 3)
	x, vonmises, meanstress, stress, strain, meanstrain, disp, hu, E, nu, mu, label, trim = bezier.eval(['x_i', 'vonmises', 'meanstress', 'stress_ij', 'strain_ij', 'meanstrain', 'u_i', 'HU', 'E', 'poisson', 'mu', 'label', 'trim'] @ omega, lhs=lhs)
	...
	# Plot Stress Results
	export.vtk('model_00_cropped_test_stress',bezier.tri,x, vonmises=vonmises, meanstress=meanstress, sigma_1=sigma_1, sigma_2=sigma_2, sigma_3=sigma_3, max_principle_stress=max_principle_stress, meanstrain=meanstrain, eps_1=eps_1, eps_2=eps_2, eps_3=eps_3, max_principle_strain=max_principle_strain, disp=disp, hu=hu, E=E, nu=nu, mu=mu, label=label, trim=trim)
