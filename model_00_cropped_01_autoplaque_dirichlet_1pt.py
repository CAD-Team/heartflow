...
def main():
    ...
    # Define Analysis
    omega.quadbasis = omega_topo.basis("spline", degree=3)
    ...
    # Stiffness Matrix
    K = omega_topo.integral("ubasis_ni,j stress_ij d:x" @ omega, degree=1)
    ...
    # Plot Stress Results
    bezier = omega_topo.sample("bezier", 3)
    x, vonmises, disp, hu, E, nu, mu, label, trim = bezier.eval(
        ["x_i", "vonmises", "u_i", "HU", "E", "poisson", "mu", "label", "trim"] @ omega,
        lhs=lhs,
    )
    export.vtk(
