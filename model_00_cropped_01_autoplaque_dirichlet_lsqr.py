...
def main():
	...
	# Plot Stress Results
	bezier = omega_topo.sample('bezier', 3)
	x, vonmises, disp, hu, E, nu, mu, label, trim = bezier.eval(['x_i', 'vonmises','u_i', 'HU', 'E', 'poisson', 'mu', 'label', 'trim'] @ omega, lhs=lhs)
	export.vtk('model_00_cropped_test_stress',bezier.tri,x, vonmises=vonmises, disp=disp, hu=hu, E=E, nu=nu, mu=mu, label=label, trim=trim)
